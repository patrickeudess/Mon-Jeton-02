(function () {
  ...
  const roles = { Président: 'Coordonne le groupe.', Secrétaire: 'Note les décisions.', Trésorier: 'Suit la caisse.', Membre: 'Épargne et participe.' };
  ...
  function show(id) {
    ...
    const membersHtml = group.members.map(member => {
      const role = T.getAvecRole(group, member.id); const selector = canRoles ? `<select aria-label="Rôle de ${esc(member.name)}" onchange="changeRole('${group.id}','${member.id}',this.value)">${Object.keys(roles).map(item => `<option ${item === role ? 'selected' : ''}>${item}</option>`).join('')}</select>` : `<span class="role">${role}</span>`;
      return `<div class="member"><div><strong>${esc(member.name)}</strong> ${member.isMe ? '<small>(moi)</small>' : ''}<small>${roles[role]} · ${T.getMemberShares(group, member.id)} part(s)</small></div>${selector}</div>`;
    }).join('');
    const loans = avec.loans.map(loan => {
      const borrower = group.members.find(item => item.id === loan.memberId); const pending = loan.status === 'requested'; const paid = loan.status === 'repaid'; const remaining = T.getLoanOutstanding(loan);
      const repayment = (!pending && !paid && (loan.memberId === me.id || canRepayments)) ? `<form class="inline-form" onsubmit="repayAvec(event,'${group.id}','${loan.id}')"><input name="amount" type="number" min="1" max="${remaining}" required placeholder="Montant remboursé"><button class="btn secondary">Enregistrer</button></form>` : '';
      return `<div class="activity"><strong>${esc(borrower ? borrower.name : 'Membre')}</strong><span>${money(loan.amount)} · ${pending ? 'Demande en attente' : paid ? 'Crédit soldé' : 'Crédit en cours'}</span>${loan.purpose ? `<small>${esc(loan.purpose)}</small>` : ''}<small>${pending ? dateTime(loan.date) : `Reste : ${money(remaining)} · échéance : ${dateOnly(loan.dueDate)}`}</small>${canLoans && pending ? `<p><button class="btn primary" onclick="approveAvec('${group.id}','${loan.id}')">Accorder le crédit</button></p>` : ''}${repayment}</div>`;
    }).join('') || '<p class="muted">Aucun crédit pour le moment.</p>';
    const addMember = canMembers ? `<details class="avec-detail"><summary>＋ Ajouter un membre</summary><form onsubmit="addAvecMember(event,'${group.id}')" class="inline-form"><input name="memberName" maxlength="80" required placeholder="Nom du nouveau membre"><button class="btn secondary">Ajouter</button></form></details>` : '';
    const institution = avec.institution && (avec.institution.name || avec.institution.project || avec.institution.reference) ? `<p class="avec-institution">${esc([avec.institution.name, avec.institution.project, avec.institution.reference].filter(Boolean).join(' · '))}</p>` : '';
    const activity = T.getCommunity(group).activity.slice(0, 12).map(item => `<div class="activity">${esc(item.message)}<small>${dateTime(item.date)}</small></div>`).join('') || '<p class="muted">Aucune action.</p>';
    document.getElementById('modal-content').innerHTML = `<section class="avec-modal-head"><div><p class="eyebrow">AVEC</p><h2>${esc(group.name)}</h2>${institution}</div><span class="role">${T.getAvecRole(group, me.id)}</span></section><section class="avec-stats"><div><small>Mes parts</small><strong>${shares}</strong><span>${money(shares * avec.shareValue)}</span></div><div><small>Caisse</small><strong>${money(T.getAvecFund(group))}</strong><span>Prêt max. : ${money(shares * avec.shareValue * 3)}</span></div></section><div class="shared-notice">Les mouvements sont visibles par les membres autorisés.</div><section class="avec-action"><label for="share-units-${group.id}">Parts à déposer</label><div><input id="share-units-${group.id}" type="number" min="1" max="20" value="1"><button class="btn primary" onclick="savePart('${group.id}')">Ajouter à la caisse</button></div></section><section class="avec-section"><div class="section-title"><h3>Membres</h3><span>${group.members.length} personne(s)</span></div>${membersHtml}${addMember}</section><details class="avec-detail"><summary>Demander un crédit</summary><form onsubmit="askLoan(event,'${group.id}')" class="avec-form-compact"><input name="amount" type="number" min="100" required placeholder="Montant souhaité"><input name="purpose" maxlength="160" placeholder="Motif (facultatif)"><button class="btn secondary">Envoyer la demande</button></form></details><details class="avec-detail" ${avec.loans.length ? 'open' : ''}><summary>Crédits du groupe <span>${avec.loans.length}</span></summary>${loans}</details><details class="avec-detail"><summary>Journal du groupe</summary>${activity}</details>`;
